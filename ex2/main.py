...
def do_colour_solve(avg_rgb, target):
    ...
    return np.linalg.pinv(tall_long_rgb_matrix.transpose() @ tall_long_rgb_matrix) @ tall_long_rgb_matrix.transpose() @ tall_target
...
if __name__ == "__main__":
    ...
    hdr_img_extra_channel = np.concatenate([hdr_img, np.ones((hdr_img.shape[0], hdr_img.shape[1], 1), dtype = np.float64)], axis = -1)
    new_hdr =  np.einsum("ij,xyj->xyi",  T, hdr_img_extra_channel)
    new_hdr = np.clip(new_hdr, a_min = 0, a_max = 1)
    white_square_values = list(new_hdr[min(square_coords[:, 0, 23]):max(square_coords[:, 0, 23]), min(square_coords[:, 1, 23]): max(square_coords[:, 0, 23]), :].mean(axis=(0, 1)))
    for i in range(3):
